...
def create_activity_graph(instances):
    ...
    Parameters:
    - instances: A list of lists, where each inner list represents a project with Activity objects.
    
    Returns:
    - A NetworkX directed graph representing activity dependencies.
    """
    G = nx.DiGraph()  # Create a directed graph

    for project in instances:
        for activity in project:
            # Add a node for each activity with labels for resources and time
            G.add_node(activity.identifier, resources=activity.resources, time=activity.time)
            
            # Add edges for each successor, creating a dependency relationship
            for successor in activity.successors:
                G.add_edge(activity.identifier, successor.identifier)
    
    return G

def draw_activity_graph(G):
    """
    Draws the activity graph with nodes and edges labeled by attributes.
    
    Parameters:
    - G: A NetworkX directed graph of activities.
    """
    pos = nx.spring_layout(G)  # Positioning of nodes

    # Draw nodes with labels including resources and time
    node_labels = {node: f'{node}\n(R: {G.nodes[node]["resources"]}, T: {G.nodes[node]["time"]})' for node in G.nodes}
...
def draw__dag(activities):
    ...
    # Add nodes for each activity with their identifier, resources, and time
    for activity in activities:
        label = f"{activity.identifier} (Time: {activity.time}, Res: {activity.resources})"
        dot.node(str(activity.identifier), label)

    # Add edges for each successor relationship, keeping track to avoid duplicates
    added_edges = set()  # Use a set to track added edges and avoid duplicates

    def add_edges(activity):
        for successor in activity.successors:
            edge = (activity.identifier, successor.identifier)
            if edge not in added_edges:  # Check if edge is already added
                dot.edge(str(activity.identifier), str(successor.identifier))
                added_edges.add(edge)  # Mark edge as added
            add_edges(successor)  # Recursive call to add successors of successors

    # Initialize adding edges from each activity in the list
    for activity in activities:
        add_edges(activity)

    # Render the graph
    dot.render('activity_graph', view=True)
...
<<<<<<< HEAD
    ...
    # result = []
    # file_pattern = "project_instances\J11x2.csv"
    # compute_times = []
    # print(0)
    ...
    #     compute_times = []
    #     total_resource, activities, alternative_chains = create_activities_from_csv(file_path)
    file_pattern = "project_instances/*.csv"
    ...
    if not files:
        print("No CSV files found in the project_instances folder.")
        return
    ...
    # Now filter the files list to include only the selected file
    files = [selected_file]

    # Process the selected file as in your original for loop
    for file_path in files:
        print(f"Processing file: {file_path}")
        result = []
        compute_times = []
        
        # Call your functions using the selected file
        total_resource, activities, alternative_chains = create_activities_from_csv(file_path)
        # print(activities)
